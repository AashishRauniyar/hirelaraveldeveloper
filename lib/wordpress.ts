...
const WORDPRESS_API_URL = process.env.WORDPRESS_URL || "https://your-wordpress-site.com/wp-json/wp/v2";
...
async function fetchAPI<T>(endpoint: string, options = {}): Promise<T> {
  ...
  if (/^https?:\/\//.test(endpoint)) {
    ...
    url = `${WORDPRESS_API_URL.replace(/\/$/, "")}${endpoint.startsWith("/") ? endpoint : `/${endpoint}`}`;
  }
...
export async function getAllPosts(filterParams?: {
  ...
  if (filterParams?.per_page) params.append("per_page", filterParams.per_page.toString());
...
export async function getPostBySlug(slug: string): Promise<Post> {
  ...
  if (!posts.length) {
    throw new WordPressAPIError(`Post not found: ${slug}`, 404, `/posts?slug=${slug}`);
  }
  return posts[0];
}
...
export async function getCategoryBySlug(slug: string): Promise<Category> {
  ...
  if (!categories.length) {
    throw new WordPressAPIError(`Category not found: ${slug}`, 404, `/categories?slug=${slug}`);
  }
  return categories[0];
}
...
export async function getTagBySlug(slug: string): Promise<Tag> {
  ...
  if (!tags.length) {
    throw new WordPressAPIError(`Tag not found: ${slug}`, 404, `/tags?slug=${slug}`);
  }
  return tags[0];
}
...
export async function getPageBySlug(slug: string): Promise<Page> {
  ...
  if (!pages.length) {
    throw new WordPressAPIError(`Page not found: ${slug}`, 404, `/pages?slug=${slug}`);
  }
  return pages[0];
}
